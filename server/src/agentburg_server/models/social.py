...
class CourtCase(Base, UUIDMixin, TimestampMixin):
    ...
    case_type: Mapped[CaseType] = mapped_column(SAEnum(CaseType, values_callable=lambda x: [e.value for e in x]), nullable=False)
    ...
    status: Mapped[CaseStatus] = mapped_column(SAEnum(CaseStatus, values_callable=lambda x: [e.value for e in x]), default=CaseStatus.FILED, nullable=False)
...
class Contract(Base, UUIDMixin, TimestampMixin):
    ...
    contract_type: Mapped[ContractType] = mapped_column(SAEnum(ContractType, values_callable=lambda x: [e.value for e in x]), nullable=False)
    ...
    status: Mapped[ContractStatus] = mapped_column(
        SAEnum(ContractStatus, values_callable=lambda x: [e.value for e in x]), default=ContractStatus.PROPOSED, nullable=False
    )
...
class Business(Base, UUIDMixin, TimestampMixin):
    ...
    business_type: Mapped[BusinessType] = mapped_column(SAEnum(BusinessType, values_callable=lambda x: [e.value for e in x]), nullable=False)
